...
#############################################################
firewall-cmd --permanent --zone=public --add-port=49226/tcp
firewall-cmd --reload
#############################################
service sshd reload
########################################################################
...
mkdir -p /root/.ssh/
##############
echo 'ssh-rsa  root@pworkserver' > /root/.ssh/authorized_keys
##############
chmod 700  /root/.ssh/
...
############################
mkdir -p /home/issteam/.ssh/
echo 'ssh-rsa  root@pworkserver' >  /home/issteam/.ssh/authorized_keys
chmod 700  /home/issteam/.ssh
chmod 400 /home/issteam/.ssh/authorized_keys
sudo chown issteam:issteam /home/issteam/.ssh -R
###############################################
/usr/bin/cp -ar /etc/sudoers /root/
############################
grep issteam /etc/sudoers
if [ `echo $?`  != 0 ] ; then
...
echo "issteam ALL=(ALL) NOPASSWD:DELEGATING" >> /etc/sudoers
fi
##################################################
